...
3. Um template por tipo de projeto (`ci-cd-node.yml`, `ci-cd-go.yml`, `ci-cd-python.yml`, `ci-cd-static.yml`, `ci-cd-monorepo.yml`) — use `bigflint` como base pro Go (é o único repo com SHA-pinning real e CodeQL já configurado).
...
4. **Lande sua primeira feature**: siga o fluxo bigpowers completo — `kickoff-branch` → `develop-tdd` → `verify-work` → `audit-code` → `release-branch`. O `release-branch` roda `scripts/land-branch.sh` (squash-merge local) ou `gh pr create` (team mode) e dispara semantic-release. O verify job nos templates `ci-cd-*.yml` roda preflight e conventional commits antes do merge. O `guard-git` bloqueia push direto no `main`.
