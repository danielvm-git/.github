...
- **Single pipeline per repo (ci → verify → deploy):** Every `ci-cd-*.yml` template follows a 3-job pipeline. The `ci` job runs lint/test/build, `verify` runs preflight + conventional commits, and `deploy` conditionally calls `bigbase-deploy`. This is one-piece flow — no handoffs between separate workflow files. See any `ci-cd-*.yml` template in `workflow-templates/`.
